...
using the {cmd:force} option with Stata's native {help merge:merge} command. When {cmd:safely()} 
is specified, {cmd:ipamerge} will assess all datasets, and for each variable, change the {help type:data type} 
to the type that will preserve the values accross all datasets. For example, assuming the "price" 
variable is numeric in the master dataset and string in the using dataset, {cmd:ipamerge} will 
convert price to a numeric dataset in the using dataset. In a situation that "price" cannot be converted to 
a numeric, then {cmd:ipamerge} will convert "price" in the master dataset to a string variable, ensuring that 
the data can be merged without loss of information.{p_end}  

{phang}
{opt outfile} must be used with the mergereport or safely option. Saves an excel workbook of the merge 
report generated by {cmd:ipamerge}. Ensure to use the extension .xlsx as .xls is has limitation that may lead to an error when generating the report{p_end}

{phang}
{opt details} must be used with the {cmd:outfile()} option. includes number and percentage of missingness and 
uniqueness to the merge report{p_end}
