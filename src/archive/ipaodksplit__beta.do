...
version 17
	...
	qui {
		...
		preserve
			...
			keep if regexm(type, "select_multiple") & !regexm(disabled, "yes|Yes|YES")
			
			* Count number of select_multiple variables
			loc sm_count `=_N'
			
			* Foreach select_multiple variable: pic out the variable name and its choice name
			forval i = 1/`sm_count' {
...
gl path "C:/Users/RAnago/Box/Az/07 - DTS/02 - ipaclean/01 - odksplit"
use "${path}/data_enrol.dta", clear


ipaodksplit using "${path}/quest_enrol.xlsx", ignore order
